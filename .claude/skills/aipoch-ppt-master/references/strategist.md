...
| Hei × song  | `SimHei, "Microsoft YaHei", sans-serif` | `SimSun, serif` | — |
...
| Adapt labels to chat language | Schema is English by default. Chinese chat → render as 「 A / / / 」. Structure stays the same; only the labels translate. |
...
**Worked example — `custom × custom`** (tail-case; e.g. deck with `#1A1A1A` + `#F5EFE0` + `#A52A2A`):

```
[Plan A] — custom × custom
 Visual: dry-brush burnt-ink, five tonal gradations, paper-grain, deliberate negative space; seal as single red mark
 Color: cream #F5EFE0 ~65% negative space + burnt-ink #1A1A1A ~20% strokes + cinnabar #A52A2A 3-5% seal
 Mood: literati restraint; like pacing
```
...
| Corporate report / analysis / | `vector-illustration` | `flat`, `editorial` |
| High-end consulting / luxury / / design-firm | `minimalist-swiss` | `editorial`, `vector-illustration` |
| Tech / SaaS / AI / | `3d-isometric`, `blueprint`, `digital-dashboard` | `flat` |
...
| Education / training / / | `sketch-notes` | `vector-illustration`, `paper-cut` |
| Children / storybook / / | `fantasy-animation` | `paper-cut`, `watercolor`, `sketch-notes` |
| Cultural / folk / festival / / | `paper-cut` | `vintage-poster`, `screen-print` |
| Methodology / Before-After / / manifesto | `ink-notes` | `editorial` |
| Government / formal / | `editorial` or `corporate-photo` | `vector-illustration` |
| Finance / journalism / | `editorial`, `digital-dashboard` | `vector-illustration` |
| Personal story / / lifestyle | `watercolor`, `warm-scene` | `corporate-photo`, `paper-cut` |
| Cultural / media / opinion / cinematic | `screen-print`, `vintage-poster` | `editorial`, `warm-scene` |
| Brand heritage / hospitality / Font size / | `vintage-poster` | `screen-print`, `editorial` |
| Gaming / retro / / | `pixel-art` | `vintage-poster` |
| Environment / wellness / | `nature` | `watercolor`, `paper-cut` |
| Classroom / blackboard / | `chalkboard` | `sketch-notes` |
...
- image_rendering_behavior: "Dry-brush burnt-ink with five tonal gradations, paper-grain at 12% opacity, deliberate negative space; seal as a single red mark; no Western outlines, no gradients."
- image_palette: custom
- image_palette_behavior: " cream `#F5EFE0` carries ~65% as negative space; burnt-ink `#1A1A1A` anchors ~20% as brush strokes; cinnabar `#A52A2A` only in 3-5% as seal. Literati restraint — no fourth color."
```
...
>
> **Source vocabulary mismatch** — the catalog is in English. When source content uses Chinese / industry jargon ("", "", "", "", ""), translate the intent first, then match against summaries. The catalog deliberately keeps no keyword index — full-read forces semantic matching rather than lexical grep.
>
