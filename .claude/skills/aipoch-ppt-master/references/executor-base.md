...
Examples: `01_Cover.svg` / `02_.svg` / `03_.svg`; `01_cover.svg` / `02_agenda.svg` / `03_key_benefits.svg`.
...
**Number readability**: TTS reads digits and symbols literally. Prefer fully-spelled forms in the language being spoken when literal pronunciation would be awkward (e.g. Chinese "" reads better than "68%"; "1-2" reads as ""). Plain integers and percentages in English are fine as-is.
...
- Adding `：① …` / `Key points: (1) …` / `：2` / `Duration: 2 minutes` / `Flex: …` lines — TTS will speak " …".
