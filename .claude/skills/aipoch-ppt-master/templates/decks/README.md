...
Deck selection is **opt-in by explicit path**. The main workflow defaults to free design. A deck is only used when the user gives an explicit directory path in their initial message (e.g. `skills/ppt-master/templates/decks//`). Bare names do not trigger. See [`SKILL.md`](../../SKILL.md) Step 3.
