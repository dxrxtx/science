...
| Transitions | Carry page-to-page flow in the opening sentence as natural prose ("……" / "Having framed X, let's turn to Y") — never bracketed `` / `[Transition]` tags |
| Plain prose only | No `#` heading line, no `- ` bullet lists, no `：① …` / `Key points:` lines, no `：2` / `Duration:` annotations — embedded notes keep them verbatim and TTS would read them aloud |
| Number readability | Spell out figures when literal TTS pronunciation is awkward (Chinese "" over "68%"; plain English integers and percentages are fine) |
