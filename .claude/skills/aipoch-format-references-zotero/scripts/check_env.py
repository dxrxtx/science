...
try:
    ...
    check('python-docx not installed', False, 'Please run: pip install python-docx')
...
if found:
    ...
    check('Pandoc not found', False, 'Please install: https://pandoc.org/installing.html')
...
if zotero_exe:
    check('Zotero installed', True, str(zotero_exe))
else:
    check('Zotero not found', False, 'Please install: https://www.zotero.org/download/')
...
try:
    ...
    s.close()
    check('Zotero is running (port 23119 open)', True,
          'References will format correctly when Refreshing in Word')
except OSError:
    check('Zotero is not running', warn=True,
          detail='Generating .docx does not require Zotero running, but Refresh in Word requires Zotero')
...
if is_zotero:
    check(f'.ris associated with Zotero ({prog_id})', True, 'Import will trigger automatically')
elif FIX_RIS and zotero_exe:
    try:
        set_ris_to_zotero(zotero_exe)
        check('.ris associated with Zotero (just modified)', True, 'Import will trigger automatically')
    except Exception as e:
    ...
    check(f'.ris not associated with Zotero ({current})', warn=True,
...
if failed == 0:
    print('Environment check passed. format-references-zotero skill is ready to use.')
else:
